...
class Model_retur_beli extends CI_Model {
    ...
    public function savedatanew($params = null, $name1 = null, $name2 = null){
        $tgl = date('d');
        $bulan = date('m');
        $tahun = date('y');
        $prinsipal = explode("~", $params->Prinsipal)[0];
        ...
        $kdCab = $this->db->query("select Kode from mst_cabang where Cabang = '".$this->cabang."' limit 1")->row();
        //create no dokumen
        $No_Usulan = 'RTR-N/'.$kdCab->Kode.'/'.$kd->Kode_Counter.'/'.$tgl.$bulan.$tahun.'/'.$lastNumber;
        ...
        $this->db->set("counter_print",0);
        $this->db->set("tipe_retur",$params->tipe_retur);
        $this->db->set("TipeRetur_2",$params->tipe_ret);
        $this->db->insert('trs_usulan_retur_beli_header');
    ...
    // ========================================= LIST DATA =====================================================
    function listdatareturbeli($search = null, $limit = null, $status = null,$tipe = ''){
        $byStatus = "";
        $byTipe = "";
        if ($tipe <> '') {
            $byTipe = " and TipeRetur_2 = '".$tipe."'";
        }
        if (!empty($status)) {
            $byStatus = " where Status_Usulan = '".$status."'";
            $sql = "SELECT *, DATE_FORMAT(added_time, '%Y-%m-%d') as tanggal FROM trs_usulan_retur_beli_header $byStatus $byTipe $search and ifnull(App_BM_Status,'') = '' order by Tanggal DESC, No_Usulan ASC $limit";
            // print_r($sql);die();
            $query = $this->db->query($sql);
        }else{
            if ($tipe <> '') {
                $byTipe = " where TipeRetur_2 = '".$tipe."'";
            }
            
            $sql = "SELECT *, DATE_FORMAT(added_time, '%Y-%m-%d') as tanggal FROM trs_usulan_retur_beli_header $byStatus $byTipe $search  order by Tanggal DESC, No_Usulan ASC $limit";
            // print_r($sql);die();
            $query = $this->db->query($sql);
        }
    ...
    public function updateDataReturBeliFromPusat()
    {   
        ...
        $TipeBKB ='';
        if ($this->db2->conn_id == TRUE) {
                $data = $this->db2->query("SELECT * FROM trs_usulan_retur_beli_header WHERE Cabang = '".$this->cabang."' AND App_Log_Status in ( 'Reject','Approve')")->result();
                foreach ($data as $dt) { //loop header
                    // header
                   
                    $cek_header = $this->db->query("select * from trs_usulan_retur_beli_header where No_Usulan = '".$dt->No_Usulan."' AND App_Log_Status = '".$dt->App_Log_Status."'")->num_rows();
                        // cek data di cabang. apakah data sudah berubah menjadi retur ?
                    if ($cek_header == 0) { // Jika data statusnya masih blm di validasi oleh pusat, lakukan update data menjadi R pada App_Pst_Status dan App_Pst_Time

                        // if($dt->Tipe_BKB == 'CN' or $dt->Tipe_BKB == 'Tolakan'){
                        $updateHeader  =  $this->db->query('update trs_usulan_retur_beli_header set Status_Usulan = "Close", App_Log_Status = "'.$dt->App_Log_Status.'", 
                            App_Log_Time = "'.date('Y-m-d H:i:s').'" where  No_Usulan = "'.$dt->No_Usulan.'"');
                
                        // }
                      
                        // print_r($this->db->error());die();
                        // detail 
                        $detail = $this->db2->query("select * from trs_usulan_retur_beli_detail where No_Usulan = '".$dt->No_Usulan."'")->result();
                        
                        foreach($detail as $r) { // loop over results
                            array_push($data_update, $r);
                            $cek = $this->db->query("select * from trs_usulan_retur_beli_detail where No_Usulan = '".$r->No_Usulan."' and Produk = '".$r->Produk."' and BatchNo ='".$r->BatchNo."' and noline ='".$r->noline."'")->num_rows();
                                ...
                                if ($cek == 0) {
                                    ...
                                    // $this->db->set("Banyak", "Banyak+".$r->Banyak, FALSE);
                                    // $this->db->set("Disc", "Disc+".$r->Disc, FALSE);
                                    // $this->db->set("Disc_Cab", "DiscCab+".$r->DiscT, FALSE);
                                    // $this->db->set("HPC", "HPC+".$r->HPC, FALSE);
                                    // $this->db->set("HPC1", "HPC1+".$r->HPC1, FALSE);
                                    // $this->db->set("Gross", "Gross+".$r->Gross, FALSE);
                                    // $this->db->set("Potongan", "Potongan+".$r->Potongan, FALSE);
                                    ...
                                    // $this->db->set("Disc_Pst", "Disc_Pst+".$r->Disc_Pst, FALSE);
                                    // $this->db->set("HPP", "HPP+".$r->HPP, FALSE);
                                    $this->db->where("No_Usulan",$r->No_Usulan);
                                    $this->db->where("Produk",$r->Produk);
                                    $this->db->where("BatchNo",$r->BatchNo);
                                     $this->db->where("noline",$r->noline);
                                    $this->db->update("trs_usulan_retur_beli_detail");
                    ...
                    $NoDoc = $dt->No_Usulan;
                    // 
                    if(count($data_update) > 0 && $dt->App_Log_Status == "Reject"){
                        $reverse_stok = $this->reverse_stok($NoDoc,"reject");
                    }
                }//loop header
