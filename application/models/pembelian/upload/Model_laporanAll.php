...
//     }
    ...
    {  
       ...
       $query = $this->db->query("SELECT  xx.Kode,xx.Pelanggan, IFNULL(SUM(IFNULL(debet_awal,0)) - SUM(IFNULL(kredit_awal,0)),0) AS Saldo_awal,IFNULL(SUM(debet),0) debet,
            ...
            CASE WHEN a.TglPelunasan < '".$tgl1."' THEN 
              CASE WHEN b.TipeDokumen = 'Retur' AND (IFNULL((IFNULL(a.`ValuePelunasan`,'') + IFNULL(a.value_pembulatan,'') + IFNULL(a.value_transfer,'') + IFNULL(a.materai,'')),0)) >= 0 THEN
                IFNULL((IFNULL(a.`ValuePelunasan`,'') + IFNULL(a.value_pembulatan,'') + IFNULL(a.value_transfer,'') + IFNULL(a.materai,'')),0) * -1
              ELSE
    ...
    public function getkartupiutangPelanggan($Kode = NULL,$tgl1 = NULL,$tgl2 = NULL){
        $query = $this->db->query("
                            ...
                            FROM trs_pelunasan_detail a 
                            JOIN (SELECT NoFaktur,STATUS,saldo,saldo_giro,tglfaktur FROM trs_faktur 
                            -- WHERE (`Saldo` + `saldo_giro`) != 0 
                            ) b
                            ...
                            FROM trs_faktur b
                            WHERE b.`Pelanggan` ='".$Kode."' 
                            -- AND ( STATUS IN ('Open', 'OpenDIH', 'Giro') OR (b.`Saldo` + b.`saldo_giro`) != 0 )
                            AND b.STATUS NOT IN ('Usulan', 'Batal', 'Reject') AND b.tglfaktur BETWEEN '".$tgl1."' AND '".$tgl2."' 
                            ORDER BY Tgl,Transaksi ASC
                    ")->result();
        return $query;
    }
