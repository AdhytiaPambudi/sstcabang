...
class Faktur extends CI_Controller {
 ...
 //    public function updateDataPelangganPusat()
	...
	public function buatalasantelatpiutang()
	{	
		if ( $this->logged ) 
		{
			$this->twig->display('pembelian/buatalasantelatpiutang.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function listalasanjto()
	{
		if ($this->logged) 
		{
			$data = $this->Model_faktur->listalasanjto();
			echo json_encode($data);
		}
		else 
		{
	...
	public function saveDataAlasanjto()
	{          
        if ($this->logged) 
		{	
			$valid=true;
			$params = (object)$this->input->post();
			$nodih = $this->Model_faktur->saveDataAlasanjto($params);
			if(empty($nodih)){
				$valid = false;
			}else{
				$valid = true;
    ...
    public function dataalasanpiutangterlambat()
	{	
		if ( $this->logged ) 
		{
			$this->twig->display('pembelian/dataalasanpiutangterlambat.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
    ...
    public function listDataAlasanJto()
	{	
		if ($this->logged) 
		{
			$data = [];
			...
			*/
			...
			/*Menghitung total data didalam database*/
			$total = $this->Model_faktur->listDataAlasanJto('','')->num_rows();
			...
			memasukan keyword didalam filed pencarian*/
			if($search!=""){
			$bySearch = " (NoFaktur like '%".$search."%' or Pelanggan like '%".$search."%'or NamaPelanggan like '%".$search."%' or Salesman like '%".$search."%' or NamaSalesman like '%".$search."%' or Status like '%".$search."%' or Status like '%".$search."%' or Total like '%".$search."%' or Saldo like '%".$search."%' or alasan_jto like '%".$search."%')";
			}
			...
			/*Urutkan dari alphabet paling terkahir*/
			// $this->db->order_by('name','asc');
			$query=$this->Model_faktur->listDataAlasanJto($bySearch, $byLimit)->result();
			
			if($search!=""){			
			$output['recordsTotal']=$output['recordsFiltered'] = $this->Model_faktur->listDataAlasanJto($bySearch, '')->num_rows();
			}
			$x = 0;
			$i = 0;
			$countCetak = 1;
