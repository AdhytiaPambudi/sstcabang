  ...
  //   } 
     public function getstokdetail()
    {
    	if ($this->logged) 
		{
	        ...
	        $Kode = $expld[0];
	        $Nama = $expld[1];
	        $tahun = date('Y',strtotime($_POST['tgldoc']));
	        $timestamp    = strtotime($_POST['tgldoc']);
			$getmonth 	= date('m', $timestamp);
			$getyear 	= date('Y', $timestamp);

			$list = $this->Model_inventori->getstokdetail($Kode,$tahun);
			$data = array();
			...
			foreach ($list as $key) {
				$row = array();
	 			...
	 			if($this->userGroup == 'Cabang'){
		 			if(date('Y-m') == $getyear."-".$getmonth or $cektglstok  == 0){
			 			$row[] = '<a class="btn btn-sm btn-primary" title="Re Proses" onclick="reproses('."'".$key->Kode_Produk."'".','."'".$key->gudang_detail."'".')" id="detail"><i class="fa fa-eye"></i>Re Proses</a>';
			 		}else{
		 				$row[] = 'Sudah Lewat Bulan';
		 			}
		 		}else{
		 			$row[] = '-';
		 		}
	...
	public function liststoksummary()
    {
    	if ($this->logged) 
		{
			$produk = $_POST['kode'];
	        $gudang = $_POST['gudang'];
	        $tgldoc = $_POST['tgl_end'];
	        $getyear = date('Y',strtotime($tgldoc));
			$list = $this->Model_inventori->liststoksummary($produk,$gudang,$getyear);
    ...
    public function updatestoksummary()
    {
    	if ($this->logged) 
		{
	        ...
	        $tgldoc = $_POST['tgl_end'];
	        $getyear = date('Y',strtotime($tgldoc));
			$valid = $this->Model_inventori->updatestoksummary($produk,$gudang,$UnitStok,$getyear);
    ...
    public function liststokdetail()
    {
    	if ($this->logged) 
		{
	        ...
	        $tgldoc = $_POST['tgl_end'];
	        $getyear = date('Y',strtotime($tgldoc));
			$list = $this->Model_inventori->liststokdetail($produk,$gudang,$getyear);
    		echo json_encode($list);
    ...
    public function updatestokdetail()
    {
    	if ($this->logged) 
		{
	        ...
	        $tgldoc = $_POST['tgl_end'];
	        $getyear = date('Y',strtotime($tgldoc));
			$valid = $this->Model_inventori->updatestokdetail($produk,$gudang,$UnitStok,$BatchNo,$BatchDoc,$getyear);
    		echo json_encode($valid);
		}
		else{
			redirect("auth/logout");
    ...
    public function getclosingstokdaily()
    {
    	if ($this->logged) 
		{
			...
			$end_date 	= date('Y-m-d');
			$getmonth 	= date('m');
			// $sel = $this->Model_inventori->getselisihdata($first_date,$end_date,$getmonth);
    ...
    public function getclosingstokbulanan()
    {
    	if ($this->logged) 
		{
	        ...
	        // $this->Model_inventori->federate_this_days();
    ...
    public function getclosingbpbDO()
    {
    	if ($this->logged) 
		{
			$status = $_POST['status'];
			...
			$list = $this->Model_inventori->getselisihbpbDO($tgl1,$tgl2,$getmonth);
			echo json_encode($list);
		}
		else{
			redirect("auth/logout");
    ...
    public function getclosingbpbDODetail()
    {
    	if ($this->logged) 
		{
			$status = $_POST['status'];
			...
			$list = $this->Model_inventori->getselisihbpbDODetail($tgl1,$tgl2,$getmonth);			
			echo json_encode($list);
		}
		else{
			redirect("auth/logout");
    ...
    public function getclosingdofaktur()
    {
    	if ($this->logged) 
		{
			$status = $_POST['status'];
			...
			$list = $this->Model_inventori->getselisihdofaktur($tgl1,$tgl2,$getmonth);			
			echo json_encode($list);
		}
		else{
			redirect("auth/logout");
    ...
    public function reproses_kartustok()
    {
    	if ($this->logged) 
		{			
			$tglDoc = $_POST['tgl_end'];
			...
			// $sawal = $this->Model_inventori->saldoawal($Kode,$getmonth,$getyear);
			$list = $this->Model_inventori->reproses_kartustok($gudang,$first_date,$end_date,$Kode,$getmonth,$getyear);
    ...
    public function reproses_stokdetail()
    {
    	if ($this->logged) 
		{			
			$status = $_POST['status'];
			...
			// $sawal = $this->Model_inventori->saldoawal($Kode,$getmonth,$getyear);
			$list = $this->Model_inventori->reproses_stokdetail_new($status,$first_date,$end_date,$Kode,$getmonth,$getyear,$saldoakhir,$stokdetail);
    ...
    public function dataFixSAwal()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			$this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/dataFixSAwal.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function getSAwalstokdetail()
    {
    	if ($this->logged) 
		{
	        $tahun = date('Y',strtotime($_POST['tgldoc']));
			...
			$tipe 		= $_POST['tipe'];
			$produk 	= $_POST['produk'];
			$list = $this->Model_inventori->getSAwalstokdetail($getmonth,$getyear,$tipe,$produk);
			$data = array();
			$x = 0;
			foreach ($list as $key) {
				$x++;
				...
				$row[] = $x;
	 			...
	 			$row[] = (!empty($key->Tahun) ? $key->Tahun : "");
	 			$row[] = (!empty($key->Bulan) ? $key->Bulan : "");
	 			$row[] = (!empty($key->KodeProduk) ? $key->KodeProduk : "");
	 			$row[] = (!empty($key->NamaProduk ) ? $key->NamaProduk  : "");
	 			$row[] = (!empty($key->Gudang ) ? $key->Gudang  : "");				
	 			$row[] = "<p align='right'>".(!empty($key->SAwalSum) ? number_format($key->SAwalSum) : 0)."</p>";
	 			$row[] = (!empty($key->GudangDetail ) ? $key->GudangDetail  : "");				
	 			$row[] = "<p align='right'>".(!empty($key->Sawaldetail) ? number_format($key->Sawaldetail) : 0)."</p>";
	 			if($this->userGroup == 'Cabang'){
	 				$row[] = '<a class="btn btn-sm btn-success" title="Fix Stok" onclick="reproses('."'".$key->Cabang."'".','."'".$key->Tahun."'".','."'".$getmonth."'".','."'".$key->KodeProduk."'".','."'".$key->Gudang."'".','."'".$x."'".')" id="summary"><i class="fa fa-eye"></i>Reproses SAwal</a>';
	 			}else{
	 				$row[] = '-';
	 			}
	 			if($this->userGroup == 'Cabang'){
	 				$row[] = '<a class="btn btn-sm btn-success" title="Fix Stok" onclick="reprosesmanual('."'".$key->Cabang."'".','."'".$key->Tahun."'".','."'".$getmonth."'".','."'".$key->KodeProduk."'".','."'".$key->Gudang."'".')" id="summary"><i class="fa fa-eye"></i>Reproses Manual</a>';
	 			}else{
	 				$row[] = '-';
	 			}
	 			...
	 			if($this->userGroup == 'Cabang'){
	 				$row[] = '<a class="btn btn-sm btn-primary" style="display:none" title="Adjustment" onclick="addjustment('."'".$key->KodeProduk."~".$key->NamaProduk."'".','."'".$key->Gudang."'".','."'".$key->HNA."'".')" id="addjustment'.$x.'"><i class="fa fa-eye"></i>Addjustment</a>';
	 			}else{
	 				$row[] = '-';
	 			}
    ...
    public function listQtySAwalDetail()
    {
    	if ($this->logged) 
		{
			$produk = $_POST['kode'];
	        ...
	        $Tahun = $_POST['Tahun'];
	        $Bulan = $_POST['Bulan'];
			$list = $this->Model_inventori->listQtySAwalDetail($produk,$gudang,$Cabang,$Tahun,$Bulan);
    		echo json_encode($list);
    ...
    public function updateSAwalDetail()
    {
    	if ($this->logged) 
		{
			$produk 	 = $_POST['KodeProduk'];
	        $gudang 	 = $_POST['Gudang'];
	        $SAwalupdate = $_POST['SAwalupdate'];
	        $BatchNo 	 = $_POST['BatchNo'];
	        $BatchDoc 	 = $_POST['BatchDoc'];
	        $Cabang 	 = $_POST['Cabang'];
	        $Tahun 		 = $_POST['Tahun'];
			...
			$valid = $this->Model_inventori->updateSAwalDetail($produk,$gudang,$SAwalupdate,$BatchNo,$BatchDoc,$Cabang,$Tahun,$Bulan);
    		echo json_encode($valid);
		}
		else{
			redirect("auth/logout");
    ...
    public function reproses_kartustok_sawal()
    {
    	if ($this->logged) 
		{			
			$tglDoc = $_POST['tgl_end'];
			...
			// $sawal = $this->Model_inventori->saldoawal($Kode,$getmonth,$getyear);
			$list = $this->Model_inventori->reproses_kartustok_sawal($gudang,$first_date,$end_date,$Kode,$getmonth,$getyear);
    ...
    public function saveMutasiKoreksi_sawal()
	{
		if ( $this->logged ) 
		{
			$valid = false;
			...
			$valid = $this->Model_inventori->saveMutasiKoreksi_sawal($params);
	...
	function kirim_inv_pusat(){
        $valid = $this->Model_main->kirim_inv_pusat();

        echo json_encode($valid);

    }
    public function sobhdetail()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			$this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/datasobhdetail.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function uploadsobhdetail()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			// $this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/Uploadsobhdetail.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function getsobhdetail()
    {
    	if ($this->logged) 
		{
			$list = $this->Model_inventori->getsobhdetail();
			$return['Result'] = $list;
    		echo json_encode($return);
    ...
    public function importsobhdetail()
    {
    	if ($this->logged) 
		{
			$params = (object)$this->input->post();
			...
			$list = $this->Model_inventori->uploadsobhdetail($name1);
			$return['Result'] = $list;
    		echo json_encode($return);
    ...
    public function datasobhdetailperiode()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			// $this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/datasobhdetailperiode.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
    ...
    public function getdatasobhdetail()
    {
    	if ($this->logged) 
		{
			$tglDoc = $_POST['tgldoc'];
	        ...
	        $timestamp    = strtotime($tglDoc);
			$transdate 	= date('Y-m-d', $timestamp);
			$list = $this->Model_inventori->getdatasobhdetail($transdate,$cabang);
			$return['Result'] = $list;
    		echo json_encode($return);
    ...
    public function sobbdetail()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			$this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/datasobbdetail.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function getsobbdetail()
    {
    	if ($this->logged) 
		{
			$list = $this->Model_inventori->getsobbdetail();
			$return['Result'] = $list;
    		echo json_encode($return);
    ...
    public function uploadsobbdetail()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			// $this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/Uploadsobbdetail.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function importsobbdetail()
    {
    	if ($this->logged) 
		{
			$params = (object)$this->input->post();
			...
			$list = $this->Model_inventori->uploadsobbdetail($name1);
			$return['Result'] = $list;
    		echo json_encode($return);
    ...
    public function datasobbdetailperiode()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			// $this->content['produk']= $this->Model_main->Produk();
			$this->twig->display('pembelian/datasobbdetailperiode.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function getdatasobbdetail()
    {
    	if ($this->logged) 
		{
			$tglDoc = $_POST['tgldoc'];
			...
			$year 	= date('Y', $timestamp);
			$month 	= date('m', $timestamp);
			$list = $this->Model_inventori->getdatasobbdetail($year,$month,$cabang);
			$return['Result'] = $list;
    		echo json_encode($return);
