...
class Model_order extends CI_Model {
    ...
    {   
        ...
        $query = $this->db->query("SELECT (CASE WHEN SUM(IFNULL(trs_invdet.UnitStok,0)) > IFNULL(invsum.UnitStok,0) THEN 0 ELSE SUM(IFNULL(trs_invdet.UnitStok,0)) END ) AS 'UnitStok'
                                    FROM trs_invdet JOIN (SELECT KodeProduk,IFNULL(UnitStok,0) AS 'UnitStok',Cabang,Gudang,Tahun
    ...
    {   
        $query = $this->db->query("select BatchNo, UnitStok, ExpDate, NoDokumen as NoBPB,UnitCOGS as 'COGS' from trs_invdet where KodeProduk = '".$kode."' and Cabang = '".$this->cabang."' and UnitStok > 0 and Gudang = 'Baik' and Tahun ='".date('Y')."' order by ExpDate,BatchNo,NoDokumen  asc")->result();
             ...
             // foreach ($so as $list) {
               ...
               if($num_so == 0){
                    ...
                    $query2 = $this->db->query("select * from trs_sales_order where NoSO = '".$list->NoSO."'")->row();
                            $this->db2->insert('trs_delivery_order_sales', $query2); 
               }else{
