...
class Model_cronjob extends CI_Model {
    ...
    function update_trs_faktur(){
        ...
        if ($this->db2->conn_id == TRUE) {
        	...
        	//=== cabang ===============
			$querytrs_faktur = $this->db->query("SELECT * FROM trs_faktur WHERE date(created_at) >= CURDATE() - INTERVAL 3 DAY or date(modified_at) >= CURDATE() - INTERVAL 3 DAY");
    ...
    function update_trs_faktur_detail(){
        ...
        if ($this->db2->conn_id == TRUE) {
        	...
        	//=== cabang ===============
			$querytrs_faktur_detail = $this->db->query("SELECT * FROM trs_faktur_detail WHERE date(created_at) >= CURDATE() - INTERVAL 3 DAY or date(modified_at) >= CURDATE() - INTERVAL 3 DAY ");
    ...
    function update_trs_invday_det(){
   	...
   	function update_trs_faktur_day(){
        ...
        if ($this->db3->conn_id == TRUE) {
        	...
        	//=== cabang ===============
        	$day = date('d');
			$queryfaktur = $this->db->query("SELECT * FROM trs_faktur_detail WHERE YEAR(TglFaktur) = YEAR(CURDATE()) AND MONTH(TglFaktur)= MONTH(CURDATE()) AND TglFaktur < CURDATE()");


			// =========== insert   =============================
			if ($queryfaktur->num_rows() > 0) {
			    $string_faktur="";
			    $jumlahbaru = 0;
			    $var_dump = "";
			      foreach ($queryfaktur->result_array() as $row) {
	                $Cabang             = ($row['Cabang'] == null) ? "null" : "'".$row['Cabang']."'";
	                $NoFaktur           = ($row['NoFaktur'] == null) ? "null" : "'".$row['NoFaktur']."'";
	                $noline             = ($row['noline'] == null) ? "null" : "'".$row['noline']."'";
	                $TglFaktur          = ($row['TglFaktur'] == null) ? "null" : "'".$row['TglFaktur']."'";
	                $TimeFaktur         = ($row['TimeFaktur'] == null) ? "null" : "'".$row['TimeFaktur']."'";
	                $Pelanggan          = ($row['Pelanggan'] == null) ? "null" : "'".$row['Pelanggan']."'";
	                $NamaPelanggan      = ($row['NamaPelanggan'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaPelanggan'])."'";
	                $AlamatFaktur       = ($row['AlamatFaktur'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['AlamatFaktur'])."'";
	                $TipePelanggan      = ($row['TipePelanggan'] == null) ? "null" : "'".$row['TipePelanggan']."'";
	                $NamaTipePelanggan  = ($row['NamaTipePelanggan'] == null) ? "null" : "'".$row['NamaTipePelanggan']."'";
	                $NPWPPelanggan      = ($row['NPWPPelanggan'] == null) ? "null" : "'".$row['NPWPPelanggan']."'";
	                $KategoriPelanggan  = ($row['KategoriPelanggan'] == null) ? "null" : "'".$row['KategoriPelanggan']."'";
	                $Acu                = ($row['Acu'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['Acu'])."'";
	                $CaraBayar          = ($row['CaraBayar'] == null) ? "null" : "'".$row['CaraBayar']."'";
	                $CashDiskon         = ($row['CashDiskon'] == null) ? "null" : "'".$row['CashDiskon']."'";
	                $ValueCashDiskon    = ($row['ValueCashDiskon'] == null) ? "null" : "'".$row['ValueCashDiskon']."'";
	                $TOP                = ($row['TOP'] == null) ? "null" : "'".$row['TOP']."'";
	                $TglJtoFaktur       = ($row['TglJtoFaktur'] == null) ? "null" : "'".$row['TglJtoFaktur']."'";
	                $Salesman           = ($row['Salesman'] == null) ? "null" : "'".$row['Salesman']."'";
	                $NamaSalesman       = ($row['NamaSalesman'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaSalesman'])."'";
	                $Rayon              = ($row['Rayon'] == null) ? "null" : "'".$row['Rayon']."'";
	                $NamaRayon          = ($row['NamaRayon'] == null) ? "null" : "'".$row['NamaRayon']."'";
	                $Status             = ($row['Status'] == null) ? "null" :  "'".$row['Status']. "'";
	                $TipeDokumen        = ($row['TipeDokumen'] == null) ? "null" :  "'".$row['TipeDokumen']. "'";
	                $KodeProduk         = ($row['KodeProduk'] == null) ? "null" :  "'".$row['KodeProduk']. "'";
	                $NamaProduk         = ($row['NamaProduk'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaProduk'])."'";
	                $UOM                = ($row['UOM'] == null) ? "null" : "'".$row['UOM']."'";
	                $Harga              = ($row['Harga'] == null) ? "null" : "'".$row['Harga']."'";
	                $QtyDO              = ($row['QtyDO'] == null) ? "null" : "'".$row['QtyDO']."'";
	                $BonusDO            = ($row['BonusDO'] == null) ? "null" : "'".$row['BonusDO']."'";
	                $QtyFaktur          = ($row['QtyFaktur'] == null) ? "null" : "'".$row['QtyFaktur']."'";
	                $BonusFaktur        = ($row['BonusFaktur'] == null) ? "null" : "'".$row['BonusFaktur']."'";
	                $ValueBonus         = ($row['ValueBonus'] == null) ? "null" : "'".$row['ValueBonus']."'";
	                $DiscCab_onf        = ($row['DiscCab_onf'] == null) ? "null" : "'".$row['DiscCab_onf']."'";
	                $ValueDiscCab_onf   = ($row['ValueDiscCab_onf'] == null) ? "null" : "'".$row['ValueDiscCab_onf']."'";
	                $DiscCab            = ($row['DiscCab'] == null) ? "null" : "'".$row['DiscCab']."'";
	                $ValueDiscCab       = ($row['ValueDiscCab'] == null) ? "null" : "'".$row['ValueDiscCab']."'";
	                $DiscCabTot         = ($row['DiscCabTot'] == null) ? "null" : "'".$row['DiscCabTot']."'";
	                $ValueDiscCabTotal  = ($row['ValueDiscCabTotal'] == null) ? "null" : "'".$row['ValueDiscCabTotal']."'";
	                $DiscPrins1         = ($row['DiscPrins1'] == null) ? "null" : "'".$row['DiscPrins1']."'";
	                $ValueDiscPrins1    = ($row['ValueDiscPrins1'] == null) ? "null" : "'".$row['ValueDiscPrins1']."'";
	                $DiscPrins2         = ($row['DiscPrins2'] == null) ? "null" : "'".$row['DiscPrins2']."'";
	                $ValueDiscPrins2    = ($row['ValueDiscPrins2'] == null) ? "null" : "'".$row['ValueDiscPrins2']."'";
	                $DiscPrinsTot       = ($row['DiscPrinsTot'] == null) ? "null" : "'".$row['DiscPrinsTot']."'";
	                $ValueDiscPrinsTotal= ($row['ValueDiscPrinsTotal'] == null) ? "null" : "'".$row['ValueDiscPrinsTotal']."'";
	                $DiscTotal          = ($row['DiscTotal'] == null) ? "null" : "'".$row['DiscTotal']."'";
	                $ValueDiscTotal     = ($row['ValueDiscTotal'] == null) ? "null" : "'".$row['ValueDiscTotal']."'";
	                $Gross              = ($row['Gross'] == null) ? "null" : "'".$row['Gross']."'";
	                $Potongan           = ($row['Potongan'] == null) ? "null" : "'".$row['Potongan']."'";
	                $Value              = ($row['Value'] == null) ? "null" : "'".$row['Value']."'";
	                $Ppn                = ($row['Ppn'] == null) ? "null" : "'".$row['Ppn']."'";
	                $LainLain           = ($row['LainLain'] == null) ? "null" : "'".$row['LainLain']."'";
	                $Total              = ($row['Total'] == null) ? "null" : "'".$row['Total']."'";
	                $Keterangan1        = ($row['Keterangan1'] == null) ? "null" : "'".$row['Keterangan1']."'";
	                $Keterangan2        = ($row['Keterangan2'] == null) ? "null" : "'".$row['Keterangan2']."'";
	                $Barcode            = ($row['Barcode'] == null) ? "null" : "'".$row['Barcode']."'";
	                $QrCode             = ($row['QrCode'] == null) ? "null" : "'".$row['QrCode']."'";
	                $NoSO               = ($row['NoSO'] == null) ? "null" : "'".$row['NoSO']."'";
	                $NoDO               = ($row['NoDO'] == null) ? "null" : "'".$row['NoDO']."'";
	                $DiscCabMax         = ($row['DiscCabMax'] == null) ? "null" : "'".$row['DiscCabMax']."'";
	                $KetDiscCabMax      = ($row['KetDiscCabMax'] == null) ? "null" : "'".$row['KetDiscCabMax']."'";
	                $DiscPrinsMax       = ($row['DiscPrinsMax'] == null) ? "null" : "'".$row['DiscPrinsMax']."'";
	                $KetDiscPrinsMax    = ($row['KetDiscPrinsMax'] == null) ? "null" : "'".$row['KetDiscPrinsMax']."'";
	                $COGS               = ($row['COGS'] == null) ? "null" : "'".$row['COGS']."'";
	                $TotalCOGS          = ($row['TotalCOGS'] == null) ? "null" : "'".$row['TotalCOGS']."'";
	                $BatchNo            = ($row['BatchNo'] == null) ? "null" : "'".$row['BatchNo']."'";
	                $ExpDate            = ($row['ExpDate'] == null) ? "null" : "'".$row['ExpDate']."'";
	                $NoBPB              = ($row['NoBPB'] == null) ? "null" : "'".$row['NoBPB']."'";
	                $Prinsipal          = ($row['Prinsipal'] == null) ? "null" : "'".$row['Prinsipal']."'";
	                $Prinsipal2         = ($row['Prinsipal2'] == null) ? "null" : "'".$row['Prinsipal2']."'";
	                $Supplier           = ($row['Supplier'] == null) ? "null" : "'".$row['Supplier']."'";
	                $Supplier2          = ($row['Supplier2'] == null) ? "null" : "'".$row['Supplier2']."'";
	                $Pabrik             = ($row['Pabrik'] == null) ? "null" : "'".$row['Pabrik']."'";
	                $Farmalkes          = ($row['Farmalkes'] == null) ? "null" : "'".$row['Farmalkes']."'";
	                $sisa_retur         = ($row['sisa_retur'] == null) ? "null" : "'".$row['sisa_retur']."'";
	                $created_at         = ($row['created_at'] == null) ? "null" : "'".$row['created_at']."'";
	                $created_by         = ($row['created_by'] == null) ? "null" : "'".$row['created_by']."'";
	                $modified_at        = ($row['modified_at'] == null) ? "null" : "'".$row['modified_at']."'";
	                $modified_by        = ($row['modified_by'] == null) ? "null" : "'".$row['modified_by']."'";
	                $kota               = ($row['kota'] == null) ? "null" : "'".$row['kota']."'";
	                $telp               = ($row['telp'] == null) ? "null" : "'".$row['telp']."'";
	                $tipe_2             = ($row['tipe_2'] == null) ? "null" : "'".$row['tipe_2']."'";
	                $region             = ($row['region'] == null) ? "null" : "'".$row['region']."'";
	                $acu2               = ($row['acu2'] == null) ? "null" : "'".$row['acu2']."'";
			      ...
			      // $this->db3->query("DELETE FROM  trs_faktur_detail_".$this->kd_cab." WHERE cabang='".$this->cabang."'");
			      $this->db3->query("DELETE FROM  trs_faktur_detail_day WHERE cabang='".$this->cabang."'");

			      $query_insert = "INSERT INTO `trs_faktur_detail_day` VALUES ".$var_dump;

			     
			      if(empty($string_faktur))  {
			      			echo "No insert found <br>";
			      			$this->db2->query("UPDATE histori_cronjob SET ket_gagal=CONCAT(ket_gagal, ' , trs_invday_det') WHERE Cabang = '".$this->cabang."' ");
			      }
			       else{
			        if ($this->db3->query($query_insert) === TRUE) {
			            echo "Berhasil insert ".$jumlahbaru ." trs_fakturday <br>";
			            $this->db2->query("UPDATE histori_cronjob SET count_table =count_table + 1, ket_berhasil =CONCAT(ket_berhasil, ' , trs_fakturday : ".$jumlahbaru."') WHERE Cabang = '".$this->cabang."' ");
			        } else {
			            echo  "gagal insert trs_fakturday ".$this->db3->error ."<br>";
			            $this->db2->query("UPDATE histori_cronjob SET ket_gagal=CONCAT(ket_gagal, ' , trs_fakturday') WHERE Cabang = '".$this->cabang."' ");
			        }
   	...
   	function update_trs_do_day(){
        ...
        if ($this->db3->conn_id == TRUE) {
        	...
        	//=== cabang ===============
        	$day = date('d');
			$querydo = $this->db->query("SELECT * FROM trs_delivery_order_sales_detail WHERE YEAR(TglDO) = YEAR(CURDATE()) AND MONTH(TglDO)= MONTH(CURDATE()) AND TglDO < CURDATE()");


			// =========== insert   =============================
			if ($querydo->num_rows() > 0) {
			    $string_do="";
			    $jumlahbaru = 0;
			    $var_dump = "";
			      foreach ($querydo->result_array() as $row) {
	                $Cabang             = ($row['Cabang'] == null) ? "null" : "'".$row['Cabang']."'";
	                $NoDO               = ($row['NoDO'] == null) ? "null" : "'".$row['NoDO']."'";
	                $TglDO              = ($row['TglDO'] == null) ? "null" : "'".$row['TglDO']."'";
	                $TimeDO             = ($row['TimeDO'] == null) ? "null" : "'".$row['TimeDO']."'";
	                $noline             = ($row['noline'] == null) ? "null" : "'".$row['noline']."'";
	                $Pengirim           = ($row['Pengirim'] == null) ? "null" : "'".$row['Pengirim']."'";
	                $NamaPengirim       = ($row['NamaPengirim'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaPengirim'])."'";
	                $Pelanggan          = ($row['Pelanggan'] == null) ? "null" : "'".$row['Pelanggan']."'";
	                $NamaPelanggan      = ($row['NamaPelanggan'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaPelanggan'])."'";
	                $AlamatKirim        = ($row['AlamatKirim'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['AlamatKirim'])."'";
	                ...
	                $ValueCashDiskon    = ($row['ValueCashDiskon'] == null) ? "null" : "'".$row['ValueCashDiskon']."'";
	                $TOP                = ($row['TOP'] == null) ? "null" : "'".$row['TOP']."'";
	                $TglJtoOrder        = ($row['TglJtoOrder'] == null) ? "null" : "'".$row['TglJtoOrder']."'";
	                $Salesman           = ($row['Salesman'] == null) ? "null" : "'".$row['Salesman']."'";
	                $NamaSalesman       = ($row['NamaSalesman'] == null) ? "null" : "'".str_replace(["'",","], " ", $row['NamaSalesman'])."'";
	                $Rayon              = ($row['Rayon'] == null) ? "null" : "'".$row['Rayon']."'";
	                $NamaRayon          = ($row['NamaRayon'] == null) ? "null" : "'".$row['NamaRayon']."'";
	                ...
	                $UOM                = ($row['UOM'] == null) ? "null" : "'".$row['UOM']."'";
	                $Harga              = ($row['Harga'] == null) ? "null" : "'".$row['Harga']."'";
	                $QtySO              = ($row['QtySO'] == null) ? "null" : "'".$row['QtySO']."'";
	                $BonusSO            = ($row['BonusSO'] == null) ? "null" : "'".$row['BonusSO']."'";
	                $QtyDO              = ($row['QtyDO'] == null) ? "null" : "'".$row['QtyDO']."'";
	                $BonusDO            = ($row['BonusDO'] == null) ? "null" : "'".$row['BonusDO']."'";
	                $ValueBonus         = ($row['ValueBonus'] == null) ? "null" : "'".$row['ValueBonus']."'";
	                $DiscCab_onf        = ($row['DiscCab_onf'] == null) ? "null" : "'".$row['DiscCab_onf']."'";
	                ...
	                $BatchNo            = ($row['BatchNo'] == null) ? "null" : "'".$row['BatchNo']."'";
	                $ExpDate            = ($row['ExpDate'] == null) ? "null" : "'".$row['ExpDate']."'";
	                $TipeFaktur         = ($row['TipeFaktur'] == null) ? "null" : "'".$row['TipeFaktur']."'";
	                $NoIDPaket          = ($row['NoIDPaket'] == null) ? "null" : "'".$row['NoIDPaket']."'";
	                $KeteranganTender   = ($row['KeteranganTender'] == null) ? "null" : "'".$row['KeteranganTender']."'";
	                $NoBPB              = ($row['NoBPB'] == null) ? "null" : "'".$row['NoBPB']."'";
	                $flag_closing       = ($row['flag_closing'] == null) ? "null" : "'".$row['flag_closing']."'";
	                $picking_list       = ($row['picking_list'] == null) ? "null" : "'".$row['picking_list']."'";
	                $BatchNoDok         = ($row['BatchNoDok'] == null) ? "null" : "'".$row['BatchNoDok']."'";
	                $BatchTglDok        = ($row['BatchTglDok'] == null) ? "null" : "'".$row['BatchTglDok']."'";
	                $Prinsipal          = ($row['Prinsipal'] == null) ? "null" : "'".$row['Prinsipal']."'";
	                $Prinsipal2         = ($row['Prinsipal2'] == null) ? "null" : "'".$row['Prinsipal2']."'";
	                $Supplier           = ($row['Supplier'] == null) ? "null" : "'".$row['Supplier']."'";
	                $Supplier2          = ($row['Supplier2'] == null) ? "null" : "'".$row['Supplier2']."'";
	                $Pabrik             = ($row['Pabrik'] == null) ? "null" : "'".$row['Pabrik']."'";
	                $Farmalkes          = ($row['Farmalkes'] == null) ? "null" : "'".$row['Farmalkes']."'";
	                $QtyDO_awal         = ($row['QtyDO_awal'] == null) ? "null" : "'".$row['QtyDO_awal']."'";
	                $BonusDO_awal       = ($row['BonusDO_awal'] == null) ? "null" : "'".$row['BonusDO_awal']."'";
	                $user_approve       = ($row['user_approve'] == null) ? "null" : "'".$row['user_approve']."'";
	                $time_approve       = ($row['time_approve'] == null) ? "null" : "'".$row['time_approve']."'";
	                $status_approve     = ($row['status_approve'] == null) ? "null" : "'".$row['status_approve']."'";
	                $user_edit          = ($row['user_edit'] == null) ? "null" : "'".$row['user_edit']."'";
	                $time_edit          = ($row['time_edit'] == null) ? "null" : "'".$row['time_edit']."'";
	                $time_batal         = ($row['time_batal'] == null) ? "null" : "'".$row['time_batal']."'";
	                $user_batal         = ($row['user_batal'] == null) ? "null" : "'".$row['user_batal']."'";
	                $created_at         = ($row['created_at'] == null) ? "null" : "'".$row['created_at']."'";
	                $created_by         = ($row['created_by'] == null) ? "null" : "'".$row['created_by']."'";
	                $modified_at        = ($row['modified_at'] == null) ? "null" : "'".$row['modified_at']."'";
	                $modified_by        = ($row['modified_by'] == null) ? "null" : "'".$row['modified_by']."'";
	                $retur_qtyDO        = ($row['retur_qtyDO'] == null) ? "null" : "'".$row['retur_qtyDO']."'";
	                $retur_bonusDO      = ($row['retur_bonusDO'] == null) ? "null" : "'".$row['retur_bonusDO']."'";
	                $status_retur       = ($row['status_retur'] == null) ? "null" : "'".$row['status_retur']."'";
	                $status_validasi    = ($row['status_validasi'] == null) ? "null" : "'".$row['status_validasi']."'";
	                $noretur            = ($row['noretur'] == null) ? "null" : "'".$row['noretur']."'";
	                $region             = ($row['region'] == null) ? "null" : "'".$row['region']."'";
	                $kota               = ($row['kota'] == null) ? "null" : "'".$row['kota']."'";
	                $telp               = ($row['telp'] == null) ? "null" : "'".$row['telp']."'";
	                $tipe_2             = ($row['tipe_2'] == null) ? "null" : "'".$row['tipe_2']."'";
			      ...
			      $var_dump=rtrim($string_do,",");
			      // $this->db3->query("DELETE FROM  trs_faktur_detail_".$this->kd_cab." WHERE cabang='".$this->cabang."'");
			      $this->db3->query("DELETE FROM  trs_delivery_order_sales_detail_day WHERE cabang='".$this->cabang."'");

			      $query_insert = "INSERT INTO `trs_delivery_order_sales_detail_day` VALUES ".$var_dump;
			      ...
			      if(empty($string_do))  {
			        ...
			        if ($this->db3->query($query_insert) === TRUE) {
			            echo "Berhasil insert ".$jumlahbaru ." trs_DOday <br>";
			            $this->db2->query("UPDATE histori_cronjob SET count_table =count_table + 1, ket_berhasil =CONCAT(ket_berhasil, ' , trs_DOday : ".$jumlahbaru."') WHERE Cabang = '".$this->cabang."' ");
			        } else {
			            echo  "gagal insert trs_DOday ".$this->db3->error ."<br>";
			            $this->db2->query("UPDATE histori_cronjob SET ket_gagal=CONCAT(ket_gagal, ' , trs_DOday') WHERE Cabang = '".$this->cabang."' ");
			        }
