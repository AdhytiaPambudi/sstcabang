...
//     }
    ...
    {   
        ...
        foreach ($query->result() as $query){
            ...
            $Produk = $this->db->query("SELECT * FROM mst_produk WHERE Kode_Produk = '".$query->KODEPROD."' LIMIT 1")->row();
            ...
            if($cekprinsinfo->num_rows() < 1){
                ...
                if($prinsinfo->Prinsipal == 'ARMOXINDO' or $prinsinfo->Prinsipal == 'DIPA REG' or $prinsinfo->Prinsipal == 'ESCOLAB'){
                    ...
                    $NilaiKlaim = ($query->UNIT * $query->HARGA) * ($query->PRINPXTRA / 100) * $prinsinfo->Nominal_klaim;
                }else{
                    // $NilaiKlaim = $query->NILAI_PRINPXTRA;
                    $NilaiKlaim = ($query->UNIT * $query->HARGA) * ($query->PRINPXTRA / 100);
                }
                if($prinsinfo->Prinsipal == 'GLOBAL HEALTH'){
                    if($Produk->Pabrik == 'LD' or $Produk->Pabrik == 'MLF' or $Produk->Pabrik == 'GHP'){
                       // $NilaiKlaim = $query->NILAI_PRINPXTRA; 
                       $NilaiKlaim = ($query->UNIT * $query->HARGA) * ($query->PRINPXTRA / 100);
                   }else{
                       $NilaiKlaim = 0;
    ...
    public function lapfakturreginkaso($search = null, $limit = null, $status = null, $tipe = null)
    {   
        $byStatus = "";
        if (!empty($status)) {
            $byStatus = "";
        ...
        if($this->cabang == "Pusat")
        {
            $query = $this->db->query("
                            select trs_faktur.* from trs_faktur
                                where (trs_faktur.`Saldo` + trs_faktur.`saldo_giro`) != 0 and 
                                    Status in ( 'Open','OpenDIH','Giro') and 
                                    TipeDokumen in ('Faktur','Retur','CN','DN') $byStatus $search order by TglFaktur DESC $limit");
        }else
        {
            if($status == "Open"){
            $query = $this->db->query("
    ...
    public function listdatarelokasidetail($search = null, $limit = null,$bydate=null)
    {   
        $byStatus = "";
        $query = $this->db->query("
                            SELECT * FROM 
                            (SELECT a.`Cabang_Pengirim`,a.`Cabang_Penerima`,
                                   ...
                                   '' AS 'No_Terima',a.`Prinsipal`,a.`Status`,
                                   a.`Produk`,a.`NamaProduk`,a.`Satuan`,
                                   a.`Qty`,a.`Bonus`,a.`Harga`,a.`Disc`,
                                   a.`BatchNo`,a.`ExpDate`,a.`BatchDoc`,
                                   a.`Gross`,a.`Potongan`,a.`Value`,a.`Ppn`,a.`Total`,
                                   a.`tgl_reject`,a.`Keterangan_reject`
                            FROM trs_relokasi_kirim_detail a
                            UNION ALL
                            SELECT b.`Cabang_Pengirim`,b.`Cabang_Penerima`,
                                   '' AS Tgl_kirim,b.`Tgl_terima` AS 'Tgl_terima',
                                   'Relokasi Terima' AS 'Jenis',b.`No_Relokasi` AS 'No_Relokasi',
                                   b.`No_Terima` AS 'No_Terima',c.`Prinsipal`,
                                   b.`Status`,b.`Produk`,b.`NamaProduk`,b.`Satuan`,
                                   b.`Qty`,b.`Bonus`,b.`Harga`,b.`Disc`,
                                   b.`BatchNo`,b.`ExpDate`,b.`BatchDoc`,
                                   b.`Gross`,b.`Potongan`,b.`Value`,b.`Ppn`,b.`Total`,
                                   '' AS 'tgl_reject','' AS 'Keterangan_reject'
                            FROM trs_relokasi_terima_detail b LEFT JOIN
                                 mst_produk c ON b.Produk = c.Kode_Produk ) relokasi
                            WHERE (CASE WHEN Jenis = 'Relokasi Terima' THEN 
                                   Tgl_terima $bydate ELSE
                                   Tgl_kirim $bydate END )
                                   $search
                            ORDER BY Tgl_terima DESC,No_Terima ASC, 
                                   Tgl_kirim DESC,No_Relokasi ASC $limit"
                                ); 
        return $query;
    }
    public function listdatarelokasiheader($search = null,$tipe=NULL, $limit = null,$bydate=null)
    {   
        if($tipe == "all"){
            $query = $this->db->query("
                            SELECT * FROM 
            ...
            $query = $this->db->query("
                            SELECT a.`Cabang_Pengirim`,a.`Cabang_Penerima`,
                                   ...
                                   a.`Keterangan`
                            FROM trs_relokasi_kirim_header a
                            where Tgl_kirim $bydate $search
                            ORDER BY Cabang_penerima,Tgl_kirim DESC,No_Relokasi ASC $limit"
                                ); 
        }elseif($tipe == "terima"){
            $query = $this->db->query("
                            SELECT b.`Cabang_Pengirim`,b.`Cabang_Penerima`,
                                   ...
                                   b.`Gross`,b.`Potongan`,b.`Value`,b.`Ppn`,b.`Total`,
                                   '' AS 'Keterangan'
                            FROM trs_relokasi_terima_header b
                            where Tgl_terima $bydate $search
                            ORDER BY Cabang_pengirim,Tgl_terima DESC,No_Terima ASC $limit"
                                ); 
        }
        
        return $query;
    }
    public function listdatarelokasiheader1($tipe=NULL,$bydate=null)
    {   
        if($tipe == "all"){
            $query = $this->db->query("
                            SELECT * FROM 
                            ...
                            WHERE (CASE WHEN Jenis = 'Relokasi Terima' THEN 
                                   ...
                                   Tgl_kirim $bydate END )
                            ORDER BY Tgl_terima DESC,No_Terima ASC, 
                                   Tgl_kirim DESC,No_Relokasi ASC"
                                )->result(); 
        }elseif($tipe == "kirim"){
            $query = $this->db->query("
                            SELECT a.`Cabang_Pengirim`,a.`Cabang_Penerima`,
                                   ...
                                   a.`Keterangan`
                            FROM trs_relokasi_kirim_header a
                            where Tgl_kirim $bydate
                            ORDER BY Cabang_penerima,Tgl_kirim DESC,No_Relokasi ASC")->result(); 
        }elseif($tipe == "terima"){
            $query = $this->db->query("
                            SELECT b.`Cabang_Pengirim`,b.`Cabang_Penerima`,
                                   '' AS Tgl_kirim,b.`Tgl_terima` AS 'Tgl_terima',
                                   ...
                                   '' AS 'Keterangan'
                            FROM trs_relokasi_terima_header b
                            where Tgl_terima $bydate
                            ORDER BY Cabang_pengirim,Tgl_terima DESC,No_Terima ASC"
                                )->result(); 
        }
        
        return $query;
    }
    public function listdatabpbheader($bydate=null)
    {   
        $query = $this->db->query("select * from trs_terima_barang_header where Cabang = '".$this->session->userdata('cabang')."' and Tipe <> 'BKB' and TglDokumen $bydate order by TglDokumen DESC")->result();
