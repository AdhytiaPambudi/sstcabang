    ...
    public function getkartustok($first_date = NULL,$end_date = NULL, $produk = NULL)
    {
      $this->Model_main->getcogssummary();
       $query = $this->db->query("SELECT NoDokumen AS 'NoDoc',
                                  ...
                                  SELECT trs_usulan_retur_beli_header.No_Usulan AS 'NoDoc', 
                                        CONCAT('Approve Usulan Beli Pusat : QTY = ',((trs_usulan_retur_beli_detail.Qty+trs_usulan_retur_beli_detail.Bonus) * -1),'  ~ Ke : ',trs_usulan_retur_beli_header.Prinsipal) AS 'Dokumen',
                                        trs_usulan_retur_beli_header.Tanggal AS 'TglDoc',
                                        trs_usulan_retur_beli_header.added_time as 'TimeDoc',
                                        trs_usulan_retur_beli_detail.Produk AS 'Kode' ,
                                        trs_usulan_retur_beli_detail.Nama_produk AS 'NamaProduk' ,
                                        ...
                                        trs_usulan_retur_beli_detail.ExpDate AS 'ExpDate'
                                  FROM  trs_usulan_retur_beli_header INNER JOIN trs_usulan_retur_beli_detail ON trs_usulan_retur_beli_header.No_Usulan = trs_usulan_retur_beli_detail.No_Usulan 
                                  WHERE ifnull(trs_usulan_retur_beli_header.App_BM_Status,'') <> 'R' and ifnull(trs_usulan_retur_beli_header.App_Log_Status,'') = 'Approve' and
                                    trs_usulan_retur_beli_detail.Produk = '".$produk ."'and 
                                        trs_usulan_retur_beli_header.tanggal between '".$first_date."' and '".$end_date."'
                                  ...
                                  SELECT trs_usulan_retur_beli_header.No_Usulan AS 'NoDoc', 
                                        ...
                                        trs_usulan_retur_beli_detail.ExpDate AS 'ExpDate'
                                  FROM  trs_usulan_retur_beli_header INNER JOIN trs_usulan_retur_beli_detail ON trs_usulan_retur_beli_header.No_Usulan = trs_usulan_retur_beli_detail.No_Usulan 
                                  WHERE ifnull(trs_usulan_retur_beli_header.App_BM_Status,'') <> 'R' and ifnull(trs_usulan_retur_beli_header.App_Log_Status,'') = 'Reject' and
                                    trs_usulan_retur_beli_detail.Produk = '".$produk ."'and 
                                        trs_usulan_retur_beli_header.tanggal between '".$first_date."' and '".$end_date."'
      ...
      foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
       {
          ...
          for($row=4; $row<=$highestRow; $row++)
          {
            ...
            $selisih = $UnitStok - $UnitOpname;
            $arrayData = $arrayData."('".date('Y')."','".$cabang."','".$noSOBH."','".$TglSOBH."','".$TimeSOBH."','".$Prinsipal."','".$KodeProduk."','".$NamaProduk."','".$UnitStok."','".$gudang."','".$UnitOpname."','".$selisih."','".$catatan."','Y','".$this->user."','".date('Y-m-d H:i:s')."','".$file_path."','Y'),";
          }
       }
       $this->db->query("delete from trs_sobh where Cabang= '".$this->cabang."' and NoSOBH = '".$noSOBH."'");
       $var_dump=rtrim($arrayData,",");
       $insert = "INSERT INTO `trs_sobh` (`Tahun`,`Cabang`,`NoSOBH`,`TglSOBH`,
                              `TimeSOBH`,`NamaPrinsipal`,`KodeProduk`,`NamaProduk`,`UnitStok`,
                              `Gudang`,`UnitOpname`,`selisih`,
                              `catatan`,`status`,`AddedUser`,`AddedTime`,
                              `file_upload`, `status_upload`
                            )VALUES".$var_dump;
       $valid = $this->db->query($insert);
       if($valid){
...
$sql = "SELECT
    ...
    public function getsobhdetail(){
        ...
        $lasttrans = $this->db->query("SELECT distinct trs_invsum.KodeProduk AS Kode_Produk,
                               ...
                               ")->result();
        // $data = array(); 
        $this->db->query("delete from trs_sobh_detail where Cabang= '".$this->cabang."' and TglSOBH = '".date('Y-m-d')."'");
        foreach ($lasttrans as $trans) {
            $kode = $trans->Kode_Produk;
            $query = $this->db->query("select distinct Tahun,
                                          '".$sobh."' as 'NoSOBH',
                                          ...
                                          '' as status
                                  from trs_invdet
                                  where KodeProduk ='".$kode."' and tahun ='".date('Y')."' and ifnull(UnitStok,0) > 0
                                  order by NamaPrinsipal,NamaProduk ASC")->result();
            foreach ($query as $key => $value) {
              $this->db->set("Cabang", $query[$key]->Cabang);
              $this->db->set("Tahun",$query[$key]->Tahun);
              ...
              $this->db->set("ValueStok", $query[$key]->ValueStok);
              $this->db->set("Gudang", $query[$key]->Gudang);
              $this->db->set("BatchNo", $query[$key]->BatchNo);
              $this->db->set("ExpDate", $query[$key]->ExpDate);
              $this->db->set("BatchDoc", $query[$key]->NoDokumen);
              $this->db->set("UnitCOGS", $query[$key]->UnitCOGS);
              $this->db->set("UnitOpname", $query[$key]->UnitOpname);
              $this->db->set("selisih", $query[$key]->selisih);
              $this->db->set("catatan", $query[$key]->catatan);
              $this->db->set("status", $query[$key]->status);
              $this->db->set("AddedUser", $this->user);
              $this->db->set("AddedTime", date('Y-m-d H:i:s'));
              $this->db->insert("trs_sobh_detail");
        ...
        $data = $this->db->query("select * from trs_sobh_detail where NoSOBH ='".$sobh."'")->result();
        return $data;

    }
    public function uploadsobhdetail($name=null){
      $file_path = "/var/www/html/sstcabang/assets/dokumen/excel/$name";   // for linux
      $noSOBH = "";
      // $file_path = "C:/xampp/htdocs/sstcabang/assets/dokumen/excel/$name";
      $FileType = pathinfo($file_path, PATHINFO_EXTENSION);
      ...
      foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
       {
          ...
          for($row=4; $row<=$highestRow; $row++)
          {
             ...
             $noSOBH = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
             $TglSOBH = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
             $TimeSOBH = date('Y-m-d H:i:s');
             $Prinsipal = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
             $KodeProduk = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
             $NamaProduk = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
             $gudang = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
             $UnitStok = $worksheet->getCellByColumnAndRow(8,$row)->getValue();
             $BatchNo = $worksheet->getCellByColumnAndRow(10,$row)->getCalculatedValue();
             $ExpDate = $worksheet->getCellByColumnAndRow(11,$row)->getValue();
             $BatchDoc = $worksheet->getCellByColumnAndRow(12,$row)->getValue();
             $UnitOpname = $worksheet->getCellByColumnAndRow(13, $row)->getCalculatedValue();
             // $selisih = $worksheet->getCellByColumnAndRow(14, $row)->getCalculatedValue();
             $catatan = $worksheet->getCellByColumnAndRow(15, $row)->getValue();
             $status = $worksheet->getCellByColumnAndRow(16, $row)->getValue();

             $UnitStok = str_replace(',','',$UnitStok);
             $UnitOpname = str_replace(',','',$UnitOpname);
             $selisih = $UnitStok - $UnitOpname;
             $arrayData = $arrayData."('".date('Y')."','".$cabang."','".$noSOBH."','".$TglSOBH."','".$TimeSOBH."','".$Prinsipal."','".$KodeProduk."','".$NamaProduk."','".$UnitStok."','".$gudang."','".$BatchNo."','".$ExpDate."','".$BatchDoc."','".$UnitOpname."','".$selisih."','".$catatan."','Y','".$this->user."','".date('Y-m-d H:i:s')."','".$file_path."','Y'),";
          }
       }
       $this->db->query("delete from trs_sobh_detail where Cabang= '".$this->cabang."' and NoSOBH = '".$noSOBH."'");
       $var_dump=rtrim($arrayData,",");
       $insert = "INSERT INTO `trs_sobh_detail` (`Tahun`,`Cabang`,`NoSOBH`,`TglSOBH`,
                              `TimeSOBH`,`NamaPrinsipal`,`KodeProduk`,`NamaProduk`,`UnitStok`,
                              `Gudang`,`BatchNo`,`ExpDate`,`BatchDoc`,`UnitOpname`,`selisih`,
                              `catatan`,`status`,`AddedUser`,`AddedTime`,
                              `file_upload`, `status_upload`
                            )VALUES".$var_dump;
       $valid = $this->db->query($insert);
       if($valid){
          $query = $this->db->query("select * from trs_sobh_detail where noSOBH = '".$noSOBH."'")->result();
       }
       return $query;
       
    }
    public function getdatasobhdetail($transdate=null,$cabang=null){
        $query = $this->db->query("select * from trs_sobh_detail where TglSOBH = '".$transdate."' and Cabang ='".$this->cabang."'")->result();
       return $query;
       
    }
    // =================== function SOBB Detail ====================================
    public function getsobbdetail(){
        $cab = $this->db->query("select Kode from mst_cabang where Cabang = '".$this->cabang."' limit 1 ")->row();
        $cab = $cab->Kode;
        $month = date('m');
        $sobb = 'SOBB/'.$cab.'/'.date('Y-m-d');
        $this->db->query("delete from trs_sobb_detail where Cabang= '".$this->cabang."' and Bulan = '".date('m')."' and Tahun = '".date('Y')."'");
        $this->Model_main->getcogssummary();
        $list = $this->db->query("select distinct Tahun,
                                          '".$month."' as Bulan,
                                          '".$sobb."' as 'NoSOBB',
                                          ...
                                          '' as status
                                  from trs_invdet
                                  where tahun ='".date('Y')."' and ifnull(UnitStok,0) > 0 and ifnull(KodeProduk,'') != ''
                                  order by NamaPrinsipal,NamaProduk ASC")->result();
        foreach ($list as $query) {
          $this->db->set("Cabang", $query->Cabang);
          $this->db->set("Tahun",$query->Tahun);
          ...
          $this->db->set("ValueStok", $query->ValueStok);
          $this->db->set("Gudang", $query->Gudang);
          $this->db->set("BatchNo", $query->BatchNo);
          $this->db->set("ExpDate", $query->ExpDate);
          $this->db->set("BatchDoc", $query->NoDokumen);
          $this->db->set("UnitCOGS", $query->UnitCOGS);
          $this->db->set("UnitOpname", $query->UnitOpname);
          $this->db->set("selisih", $query->selisih);
          $this->db->set("catatan", $query->catatan);
          ...
          $this->db->set("AddedUser", $this->user);
          $this->db->set("AddedTime", date('Y-m-d H:i:s'));
          $this->db->insert("trs_sobb_detail");
        }
        return $list;
    }
    public function uploadsobbdetail($name=null){
      $file_path = "/var/www/html/sstcabang/assets/dokumen/excel/$name";   // for linux
      $noSOBB = "";
      // $file_path = "C:/xampp/htdocs/sstcabang/assets/dokumen/excel/$name";
      $FileType = pathinfo($file_path, PATHINFO_EXTENSION);
      ...
      foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
       {
          ...
          for($row=4; $row<=$highestRow; $row++)
          {
             ...
             $noSOBB = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
             $TglSOBB = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
             $TimeSOBB = date('Y-m-d H:i:s');
             $Prinsipal = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
             $KodeProduk = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
             $NamaProduk = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
             $gudang = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
             $UnitStok = $worksheet->getCellByColumnAndRow(8,$row)->getValue();
             $UnitCOGS = $worksheet->getCellByColumnAndRow(9,$row)->getValue();
             $BatchNo = $worksheet->getCellByColumnAndRow(11,$row)->getCalculatedValue();
             $ExpDate = $worksheet->getCellByColumnAndRow(12,$row)->getValue();
             $BatchDoc = $worksheet->getCellByColumnAndRow(13,$row)->getValue();
             $UnitOpname = $worksheet->getCellByColumnAndRow(14, $row)->getCalculatedValue();
             // $selisih = $worksheet->getCellByColumnAndRow(14, $row)->getCalculatedValue();
             $catatan = $worksheet->getCellByColumnAndRow(16, $row)->getValue();
             $status = $worksheet->getCellByColumnAndRow(17, $row)->getValue();

             $UnitStok = str_replace(',','',$UnitStok);
             $UnitOpname = str_replace(',','',$UnitOpname);
             $selisih = $UnitStok - $UnitOpname;
             $arrayData = $arrayData."('".date('Y')."','".$cabang."','".date('m')."','".$noSOBB."','".$TglSOBB."','".$TimeSOBB."','".$Prinsipal."','".$KodeProduk."','".$NamaProduk."','".$UnitStok."','".$UnitCOGS."','".$gudang."','".$BatchNo."','".$ExpDate."','".$BatchDoc."','".$UnitOpname."','".$selisih."','".$catatan."','Y','".$this->user."','".date('Y-m-d H:i:s')."','".$file_path."','Y'),";
          }
       }
       $this->db->query("delete from trs_sobb_detail where Cabang= '".$this->cabang."' and NoSOBB = '".$noSOBB."'");
       $var_dump=rtrim($arrayData,",");
       $insert = "INSERT INTO `trs_sobb_detail` (`Tahun`,`Cabang`,`Bulan`,`NoSOBB`,`TglSOBB`,
                              `TimeSOBB`,`NamaPrinsipal`,`KodeProduk`,`NamaProduk`,`UnitStok`,`UnitCOGS`,
                              `Gudang`,`BatchNo`,`ExpDate`,`BatchDoc`,`UnitOpname`,`selisih`,
                              `catatan`,`status`,`AddedUser`,`AddedTime`,
                              `file_upload`, `status_upload`
                            )VALUES".$var_dump;
       $valid = $this->db->query($insert);
       if($valid){
          $query = $this->db->query("select * from trs_sobb_detail where noSOBB = '".$noSOBB."'")->result();
       }
       return $query;
       
    }
    public function getdatasobbdetail($year=null,$month=null,$cabang=null){
        $query = $this->db->query("select * from trs_sobb_detail where Tahun = '".$year."' and Bulan = '".$month."' and Cabang ='".$this->cabang."'")->result();
       return $query;
