...
class Model_usulanBeli extends CI_Model {
    ...
    {
      $prinsipal = "";
      $getprins = $this->db->query("SELECT Prinsipal FROM mst_produk WHERE Kode_Produk='".$kode."' limit 1")->row();
      if($getprins != ""){
        $prinsipal = $getprins->Prinsipal;
      }
      if($prinsipal == 'ANDALAN' or $prinsipal == 'SOLAS REG' or $prinsipal == 'SOLAS N.REG'){
        $query = $this->db->query("SELECT SUM(vBeli) AS vBeli 
                                  FROM (SELECT (SUM(Value_Usulan)) AS vBeli 
                                         ...
                                         WHERE cabang='".$this->session->userdata('cabang')."' AND 
                                               Prinsipal = '".$prinsipal."' AND 
                                               trs_usulan_beli_detail.`Produk` ='".$kode."' AND 
                                               (status_usulan NOT IN ('Tolak','Closed','Batal') and 
                                             trs_usulan_beli_detail.`No_Usulan` NOT IN (SELECT IFNULL(trs_po_header.`No_Usulan`,'') AS 'No_Usulan' FROM trs_po_header where trs_po_header.Prinsipal = '".$prinsipal."')) AND 
                                        ...
                                        WHERE cabang='".$this->session->userdata('cabang')."' AND  
                                            ...
                                            (status_pr NOT IN ('Tolak','Closed','Batal') and 
                                                  trs_pembelian_detail.`No_PR` NOT IN (SELECT IFNULL(trs_po_header.`No_PR`,'') AS 'No_PR' FROM trs_po_header where trs_po_header.Prinsipal = '".$prinsipal."')) AND 
                                        ...
                                        FROM `trs_po_detail` 
                                        WHERE cabang='".$this->session->userdata('cabang')."' AND  
                                              trs_po_detail.Prinsipal = '".$prinsipal."' AND 
    ...
    public function getNilaiBeliUsulanBelibyPrins($prinsipal = NULL)
    {
       $query = $this->db->query("SELECT SUM(vBeli) AS vBeli
                                    ...
                                    ")->row();
        $query1 = $this->db->query("SELECT SUM(vBeli) AS vBeli
                                    FROM (SELECT (SUM(Value_Usulan)) AS vBeli 
                                          FROM `trs_usulan_beli_detail` 
                                          WHERE cabang='".$this->session->userdata('cabang')."' AND  
    ...
    public function getLimitPrins($kode = NULL)
    {
        ...
        if($prinsipal == 'ANDALAN' or $prinsipal == 'SOLAS REG' or $prinsipal == 'SOLAS N.REG' ){
          ...
          $query = $this->db->query("select Limit_Beli as xlimit from mst_limit_pembelian_cabang where Cabang ='".$this->session->userdata('cabang')."' and MONTH(Bulan)='".$bln."' and YEAR(Bulan)='".$thn."' limit 1")->row(); 
        }
    ...
    public function getLimitPrinsbyprins($prinsipal = NULL)
    {
        ...
        $thn = date("Y");
        $query = $this->db->query("select Limit_Beli as xlimit from mst_limit_pembelian where Cabang ='".$this->session->userdata('cabang')."' and Prinsipal ='".$prinsipal."' and MONTH(Bulan)='".$bln."' and YEAR(Bulan)='".$thn."' limit 1")->row(); 
        $query1 = $this->db->query("select Limit_Beli as xlimit from mst_limit_pembelian_cabang where Cabang ='".$this->session->userdata('cabang')."' and MONTH(Bulan)='".$bln."' and YEAR(Bulan)='".$thn."' limit 1")->row(); 
        $arr_query = [
                      'lprins' => $query,
                      'lcab' => $query1
                    ];
    ...
    public function listProdukOrderMonitor($prinsipal = NULL,$ltime=null)
    {
        ...
        $query = $this->db->query("select distinct mst_produk.*,
                                           ...
                                           round(((0.75 + ('".$ltime."'/30)) - ((UnitStok+ifnull(QtyGIT,0)+ifnull(QtyGITSP,0))/ifnull(avgjual,0))) * ifnull(avgjual,0),2) as 'pesan'
                                    from mst_produk 
                                    left join 
                                    (select KodeProduk,
                                    ...
                                    left join
                                    (SELECT sgit.produk,SUM(IFNULL(sgit.QtyGIT,0)) AS QtyGIT ,SUM(IFNULL(sgit.QtyGITSP,0)) AS QtyGITSP 
                                       FROM ( SELECT produk,(CASE WHEN IFNULL(trs_po_detail.Sisa_PO,0) > 0 THEN IFNULL(Sisa_PO,'') ELSE (IFNULL(Qty_PO,'') + IFNULL(Bonus,'')) END) AS QtyGIT , 0 AS QtyGITSP
                                              FROM `trs_po_detail` left join mst_prinsipal
                                              on trs_po_detail.Prinsipal = mst_prinsipal.Prinsipal
                                              WHERE cabang='".$this->cabang."' AND trs_po_detail.Prinsipal ='".$prinsipal."' AND 
                                                    ...
                                                    UNION ALL
                                    SELECT Produk, Banyak AS QtyGIT, 0 AS QtyGITSP
                                    FROM trs_delivery_order_detail left join mst_prinsipal
                                    ON trs_delivery_order_detail.`Prinsipal` = mst_prinsipal.Prinsipal
                                    WHERE trs_delivery_order_detail.`Cabang` = '".$this->cabang."' AND 
                                          trs_delivery_order_detail.`Status` = 'Open' AND 
                                          ...
                                          (CASE WHEN mst_prinsipal.umur_GIT = '0' THEN trs_delivery_order_detail.`TglDokumen` BETWEEN '".$bulanini."' AND '".$bulanini_akhir."'
                                           WHEN mst_prinsipal.umur_GIT = '1' THEN trs_delivery_order_detail.`TglDokumen` BETWEEN '".$satubulan_awal."' AND '".$bulanini_akhir."'
                                           WHEN mst_prinsipal.umur_GIT = '2' THEN trs_delivery_order_detail.`TglDokumen` BETWEEN '".$duabulan."' AND '".$bulanini_akhir."' ELSE
                                                trs_delivery_order_detail.`TglDokumen` <= '".$bulanini_akhir."' END)

                                                UNION ALL

                                      SELECT trs_pembelian_pusat_detail.Produk, trs_pembelian_pusat_detail.Banyak * -1 AS QtyGIT,trs_pembelian_pusat_detail.Banyak AS QtyGITSP
                                    FROM trs_delivery_order_detail LEFT JOIN mst_prinsipal
                                    ON trs_delivery_order_detail.`Prinsipal` = mst_prinsipal.Prinsipal
                                    LEFT JOIN trs_pembelian_pusat_detail ON trs_pembelian_pusat_detail.NoDokumen = trs_delivery_order_detail.NoPO 
                                    AND trs_pembelian_pusat_detail.Produk = trs_delivery_order_detail.Produk
                                    WHERE trs_pembelian_pusat_detail.`Cabang` = '".$this->cabang."' AND 
                                           trs_delivery_order_detail.`Status` = 'Open' AND 
                                          LEFT(trs_delivery_order_detail.NoPO,2) ='SP' AND 
                                          trs_pembelian_pusat_detail.Prinsipal ='".$prinsipal."' AND 
                                          (CASE WHEN mst_prinsipal.umur_GIT = '0' THEN trs_pembelian_pusat_detail.`TglDokumen` BETWEEN '".$bulanini."' AND '".$bulanini_akhir."'
                                           WHEN mst_prinsipal.umur_GIT = '1' THEN trs_pembelian_pusat_detail.`TglDokumen` BETWEEN '".$satubulan_awal."' AND '".$bulanini_akhir."'
                                           WHEN mst_prinsipal.umur_GIT = '2' THEN trs_pembelian_pusat_detail.`TglDokumen` BETWEEN '".$duabulan."' AND '".$bulanini_akhir."' ELSE
                                                trs_pembelian_pusat_detail.`TglDokumen` <= '".$bulanini_akhir."' END)
                                                GROUP BY trs_delivery_order_detail.NOPO ,trs_delivery_order_detail.Produk
    ...
    {   
        ...
        if ($this->db2->conn_id == TRUE) {
          ...
          $nomor = $this->db2->query("select No_Usulan,Status_Usulan from trs_usulan_beli_header where Cabang = '".$this->session->userdata('cabang')."' and Status_Usulan not in ('Usulan') and date(added_time) between '".$satubulan_awal."' and '".date('Y-m-d')."' ")->result();
          foreach ($nomor as $no) {
            $this->db->set('Status_Usulan', $no->Status_Usulan);
            $this->db->where('No_Usulan', $no->No_Usulan);
            $this->db->where('Status_Usulan', "Usulan");
            // $this->db->where('statusPusat', 'Berhasil');
            $this->db->update('trs_usulan_beli_header'); // 
          }
    ...
    public function load_datadopusat($search=null, $limit=null, $status=null){
        $header = $this->db->query("select * from trs_delivery_order_header where Cabang='".$this->cabang."' $search order by TglDokumen Desc, NoDokumen Asc $limit");
        return $header;
    }
    ...
    public function updatedataDOpusat(){
        $this->db2 = $this->load->database('pusat', TRUE);      
        if ($this->db2->conn_id == TRUE) {
            $doheader = $this->db2->query("select * from trs_delivery_order_header where Cabang='".$this->cabang."' and Status= 'Open' and ifnull(No_Relokasi,'') = '' AND TglDokumen >= CURDATE() - INTERVAL '3' MONTH")->result();
            foreach($doheader as $r) {
                ...
                // if ($cekpo > 0) {
                    ...
                    // ==== BACA DETAIL ====
                    $dodetail = $this->db2->query("select * from trs_delivery_order_detail where Cabang='".$this->cabang."' and NoDokumen = '".$r->NoDokumen."' and status ='Open'")->result();
                    if($cekdo==0){
    ...
    public function setStok($params = NULL)
    {
        ...
        $valuestok = $params->Value;
        $jml_qty = $params->Banyak;
        $COGS = $params->Value/$jml_qty;
    ...
    public function closedataDO($No=null,$cabang=null,$status=null)
    {
      $this->db2 = $this->load->database('pusat', TRUE);
      if ($this->db2->conn_id == TRUE) {
        if($status == 'Closed'){
            ...
            $this->db->query("update trs_delivery_order_detail
                        set Status='Closed'
                        where NoDokumen ='".$No."' and Cabang ='".$cabang."'");
            if($valid){
              $valid = $this->db2->query("update trs_delivery_order_detail
                        set Status='Closed'
                        where NoDokumen ='".$No."' and Cabang ='".$cabang."'");
              $valid = $this->db2->query("update trs_delivery_order_header
                        set Status='Closed'
                        where NoDokumen ='".$No."' and Cabang ='".$cabang."'");
            }
        }elseif($status == 'Batal'){
            ...
            if($valid){
                ...
                $valid = $this->db2->query("update trs_delivery_order_header
                        set Status='Batal'
                        where NoDokumen ='".$No."' and Cabang ='".$cabang."'");
                $ceksj = $this->db2->query("select * from trs_delivery_order_header where NoDokumen ='".$No."' and Cabang ='".$cabang."' and ifnull(flag_suratjalan,'') ='Y' limit 1")->row();
                $nosj = $ceksj->NoSJ;
                $valid = $this->db2->query("update trs_surat_jalan
                        set Status='Open'
                        where NoDokumen ='".$nosj."' and Cabang ='".$cabang."'");
            }
        }
      }
    ...
    public function UpdateValdataDO($No=null,$cabang=null,$status=null)
    {
      $this->db2 = $this->load->database('pusat', TRUE);
      if ($this->db2->conn_id == TRUE) {
        $dopusatheader =$this->db2->query("select * 
                            from  trs_delivery_order_header
                            where NoDokumen ='".$No."' and 
                            Cabang ='".$cabang."' limit 1")->row();
        $dopusatdetail =$this->db2->query("select * 
                            from  trs_delivery_order_detail
                            where NoDokumen ='".$No."' and 
                            Cabang ='".$cabang."'")->result();

        $this->db->set("Gross", $dopusatheader->Gross);
        $this->db->set("Potongan", $dopusatheader->Potongan);
        $this->db->set("Value", $dopusatheader->Value);
        $this->db->set("PPN", $dopusatheader->PPN);
        $this->db->set("Total", $dopusatheader->Total);
        $this->db->set("TotalBiaya", $dopusatheader->TotalBiaya);
        $this->db->set("TimeEdit", date('Y-m-d H:i:s'));
        $this->db->set("UserEdit", $this->session->userdata('username'));
        $this->db->where("NoDokumen", $No);
        $this->db->where("Cabang", $cabang);
        ...
        foreach ($dopusatdetail as $detail) {
          $this->db->set("Disc", $detail->Disc);
          $this->db->set("HrgBeli", $detail->HrgBeli);
          $this->db->set("DiscT", $detail->DiscT);
          $this->db->set("Disc_Pst", $detail->Disc_Pst);
          $this->db->set("Harga_Beli_Pst", $detail->Harga_Beli_Pst);
          $this->db->set("Gross", $detail->Gross);
          $this->db->set("Potongan", $detail->Potongan);
          $this->db->set("Value", $detail->Value);
          $this->db->set("PPN", $detail->PPN);
          $this->db->set("Total", $detail->Total);
          $this->db->set("TotalBiaya", $detail->TotalBiaya);
          $this->db->set("TimeEdit", date('Y-m-d H:i:s'));
          $this->db->set("UserEdit", $this->session->userdata('username'));
          $this->db->where("noline", $detail->noline);
          $this->db->where("Produk", $detail->Produk);
