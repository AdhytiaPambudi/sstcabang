...
class Usulanreturbeli extends CI_Controller {
	...
	public function listdatareturbeli($cek = null){
		...
		if($cek=='all')
			$total = $this->Model_retur_beli->listdatareturbeli('','','',$tipe)->num_rows();
		else
			$total = $this->Model_retur_beli->listdatareturbeli('','','Open',$tipe)->num_rows();
		...
		if($cek=='all')
				$query=$this->Model_retur_beli->listdatareturbeli($bySearch,$byLimit,'',$tipe)->result();
			else
				$query=$this->Model_retur_beli->listdatareturbeli($bySearch, $byLimit, "Open",$tipe)->result();
		
		if($search!=""){			
			if($cek=='all')
				$output['recordsTotal']=$output['recordsFiltered'] = $this->Model_retur_beli->listdatareturbeli($bySearch,'','',$tipe)->num_rows();
			else
				$output['recordsTotal']=$output['recordsFiltered'] = $this->Model_retur_beli->listdatareturbeli($bySearch,'', "Open",$tipe)->num_rows();
		}
		...
		foreach ($query as $baris) {
				...
				$row[] = (!empty($baris->No_Usulan) ? $baris->No_Usulan : "");
	 			...
	 			$row[] = (!empty($baris->App_Log_Status) ? $baris->App_Log_Status : "Pending");
	 			// $row[] = (!empty($baris->App_BM_Time) ? $baris->App_BM_Time : "");
	 			$row[] = (!empty($baris->tipe_retur) ? $baris->tipe_retur : "");
	 			$row[] = (!empty($baris->tipe_retur) ? $baris->TipeRetur_2 : "");
...
// ======================================================================================================================
	...
	public function saveusulanreturnew()
	{
		...
		{	
			...
			if($cektglstok == 1){
				if(($dailystok->tgl_daily_closing == $date) and ($dailystok->flag_trans == 'Y') and ($dailystok->flag_stok == 'Y')){
					...
					if($respon == true){
						$respon = $this->Model_retur_beli->savedatanew($params, $name1, $name2);
					}else{
						$respon = false;
					}
            		echo json_encode($respon);
    ...
    public function updateDataReturBeliFromPusat()
    {
        ...
        if ($this->logged) 
        {
            $status = $this->Model_retur_beli->updateDataReturBeliFromPusat();
            echo json_encode(array("status" => TRUE, "ket" => $status));
        }
        else 
        {   
