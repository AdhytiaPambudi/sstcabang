...
class Order extends CI_Controller {
		...
		{ 
		  ...
		  if($data != ""){
		  	...
		  	if ($get_onf->num_rows() > 0) {
			  	if ($satuan->flag_prins_onf == 'Y') {
				  	$data->Cab_onf = 0;
				  	$data->Prins_onf = $get_onf->row("Prins_Onf");
				  	$data->Total_onf = $get_onf->row("Prins_Onf");
		  		}else{
		  			if(empty($discprinsNew->No_Usulan)){
		  				$data->Cab_onf = $get_onf->row("Prins_Onf");
		  				$data->Prins_onf = 0;
		  			}else{
		  				$data->Prins_onf = $get_onf->row("Prins_Onf");
		  				$data->Cab_onf = 0;
	...
	public function gethargaproduk()
	{
		if ($this->logged) 
		{
			$data = $this->Model_order->gethargaproduk();
			echo json_encode($data);
		}
		else 
		{
	...
	function getDetailSO(){
		...
		if ($status != 'Y') {
			$data = $this->Model_order->setApprovaEditBatch($no);
		}else{
			$data = $this->Model_order->getDetailSO($no);
		}
	...
	function getnoBatch(){
		...
		$data = $this->Model_order->getnoBatch($KodeProduk);

		echo json_encode($data);
	}

	function SaveDOManual(){
			...
			if($cektglstok == 1){
				if(($dailystok->tgl_daily_closing == $date) and ($dailystok->flag_trans == 'Y') and ($dailystok->flag_stok == 'Y')){
					...
					if($cek == '0'){
						...
						$valid = $this->Model_order->SaveDOManual($params);
						...
						$valid=false;
						$x =  ['status'=>$valid, 'pesan'=>'Silahkan Unlock Transaksi terlebih dulu','user'=>$this->user];
						
					}
				}else{
					$valid=false;
					$x =  ['status'=>$valid, 'pesan'=>'Info! transaksi Close karena settlement bulanan sudah dilakukan, atau belum closing bulanan','user'=>$this->user];
				}
	...
	function cetak_so(){
		...
		$data['data'] = $this->Model_order->getDetailSO($no);
		$this->load->view('../../assets/print_template/cetak_so', $data);
	}
}
