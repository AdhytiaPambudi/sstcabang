...
class Prinsipal_limit extends CI_Controller {
	...
	public function usulanlimitbelicabang()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			// $this->content['limitcabang']= $this->Model_prinsipal_limit->getLimitCabang();
			// $this->content['limitbelicabang']= $this->Model_prinsipal_limit->getLimitCabangOutstanding();
			$this->twig->display('pembelian/buat_usulan_cabang_limit.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function listDataCabangLimit()
	{	
		$data = "";
		...
		$list = $this->Model_prinsipal_limit->getlistdatalimitcabang($bySearch);
        $data = array();
        $x = $_POST['start'];
		foreach ($list as $prinsipal_limit) {
			$x++;
			$row = array();
			$row[] = $x;
			$row[] = '<a class="btn btn-sm btn-success" href="javascript:void(0)" title="Kirim Data Ke Pusat" onclick="kirimdata('."'".$prinsipal_limit->id."'".')"><i class="fa fa-trash"></i> Kirim ke Pusat</a>';
 			$row[] = (!empty($prinsipal_limit->Cabang) ? $prinsipal_limit->Cabang : "");
 			$row[] = (!empty(number_format($prinsipal_limit->Limit)) ? number_format($prinsipal_limit->Limit) : "0");
 			$row[] = (!empty(number_format($prinsipal_limit->Limit_Outstanding)) ? number_format($prinsipal_limit->Limit_Outstanding) : "0");
 			$row[] = (!empty(number_format($prinsipal_limit->Limit_Usulan))? number_format($prinsipal_limit->Limit_Usulan) : "0");
 			$row[] = (!empty($prinsipal_limit->status) ? $prinsipal_limit->status : "");
 			$row[] = (!empty($prinsipal_limit->Keterangan_Usulan) ? $prinsipal_limit->Keterangan_Usulan : "");
 			$row[] = (!empty($prinsipal_limit->Waktu_Usulan) ? $prinsipal_limit->Waktu_Usulan : "");
 			// $row[] = (!empty($prinsipal_limit->Waktu_Approv) ? $prinsipal_limit->Waktu_Approv : "NULL");
			if($prinsipal_limit->Approve_BM == 'Approve' or $prinsipal_limit->Approve_RBM == 'Approve' or $prinsipal_limit->Approve_NSM == 'Approve' or $prinsipal_limit->Approve_pusat == 'Approve'){
				$row[] = "-";
				$row[] = "-";
				...
				$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="updateData('."'".$prinsipal_limit->id."'".')"><i class="fa fa-pencil"></i> Edit</a>
				  ';
				$row[] = '<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="deleteData('."'".$prinsipal_limit->id."'".')"><i class="fa fa-trash"></i> Delete</a>';
			}
	...
	public function getCabangLimit()
	{
		if ( $this->logged ) 
		{
				$q = $this->Model_prinsipal_limit->getLimitCabang();
				$q2 = $this->Model_prinsipal_limit->getLimitCabangOutstanding();

				$limit = (!empty($q->Limit_Beli) ? $q->Limit_Beli : 0);
				$outstanding = (!empty($q2->vBeli) ? $q2->vBeli : 0);
	...
	public function addDataCabangLimit()
	{
		if ( $this->logged ) 
		{	
			//$this->_validate();
			$params = (object)$this->input->post();
			$data = $this->Model_prinsipal_limit->saveCabangLimit($params);
	...
	public function getDataCabangLimit($id = NULL)
	{    
		if ( $this->logged ) 
		{
			$valid = $this->Model_prinsipal_limit->getdataCabangById($id); 
			echo json_encode($valid);
		}
		else 
		{	
	...
	public function updateDataCabangLimit()
	{
		if ( $this->logged  ) 
		{	
			//$this->_validate();
			$params = (object)$this->input->post();
			$valid = $this->Model_prinsipal_limit->updateDataCabangLimit($params);
			echo json_encode(array("status" => TRUE));
		}
		else 
		{	
	...
	public function datalimitcabangbeli()
	{	
		if ( $this->logged ) 
		{
			$this->twig->display('pembelian/approvallimitcabangbeli.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function listDatalimitbelicabang($cek = null)
    {   
 		$list = $this->Model_prinsipal_limit->listDatalimitbelicabang($cek)->result();
        $data = array();
		foreach ($list as $key) {
			$row = array();
 			...
 			if($cek !='all'){
	 			if ($this->userGroup == 'BM'){
	 				...
	 				if($key->Approve_BM != ""){
	 					...
	 					$row[] = '<a class="btn btn-sm btn-info" title="Approve" onclick="prosesData('."'".$key->id."'".','."'Approve'".')" id="approve"><i class="fa fa-eye"></i> Approve</a>&nbsp;&nbsp;<a class="btn btn-sm btn-warning" title="Reject" onclick="prosesData('."'".$key->id."'".','."'Reject'".')" id="reject"><i class="fa fa-eye"></i> Reject</a>'; 
	 				}	 				
		 		}else{
    ...
    public function prosesDatalimitbelicabang()
	{          
        if ($this->logged) 
		{	
			$No = $_POST['No'];
			$status = $_POST['status'];
			$valid = $this->Model_prinsipal_limit->prosesDatalimitbelicabang($No,$status);
    ...
    public function updateDataLimitBeliCabangPusat()
	{
		...
		{
			// $no = $_POST['no'];
			$data = $this->Model_prinsipal_limit->updateDataLimitBeliCabangPusat();
			echo json_encode($data);
		}
		else 
		{	
	...
	public function datalimitbelicabang()
	{	
		if ( $this->logged ) 
		{
			$this->content['cabang']= $this->Model_main->Cabang();
			$this->twig->display('pembelian/data_cabang_limit.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	public function listdataCabangLimitbeli()
    {  
    	$data = "";
        ...
        $list = $this->Model_prinsipal_limit->listCabangLimitbeli($bySearch);
        $data = array();
        $x = $_POST['start'];
		foreach ($list as $prinsipal_limit) {
			$x++;
			...
			$row[] = $x;
			$row[] = (!empty($prinsipal_limit->Cabang) ? $prinsipal_limit->Cabang : "");
			$row[] = (!empty($prinsipal_limit->Bulan) ? $prinsipal_limit->Bulan : "");
 			$row[] = "<p align='right'>".(!empty($prinsipal_limit->Limit_Beli) ? number_format($prinsipal_limit->Limit_Beli) : "0")."</p>";	
			$data[] = $row;
		}
    ...
    public function deleteDataCabangLimit($id = NULL)
	{
		$this->Model_prinsipal_limit->deleteCabangLimit($id);
		echo json_encode(array("status" => TRUE));
	}
	public function prosesKirimDataLimitCabang()
	{          
        if ($this->logged) 
		{	
			$id = $_POST['id'];
			$valid = $this->Model_prinsipal_limit->prosesKirimDataLimitCabang($id);
			echo json_encode(array("status" => $valid));
		}
		else 
		{	
