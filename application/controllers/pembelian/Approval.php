...
class Approval extends CI_Controller {
	...
	// View Page Data Order Khusus - @DnA
	public function approvalOrderKhusus()
	{	
		if ( $this->logged ) 
		{
			$this->twig->display('pembelian/dataOrderKhusus.html', $this->content);
		}
		else 
		{
			redirect("auth/logout");
	...
	// View List Order Khusus - @DnA
	public function listDataApprovalOrderKhusus()
	{
		$byLimit = "";
		$byfilter = "";
		...
		$data_api = json_decode($this->Model_approval->getDataApprovalOrderKhusus(),TRUE);
		...
		$nomor_urut=$start+1;
		if ($query <> 'Data Kosong') {
			foreach ($query as $baris => $row) {
				if ($this->userGroup == 'BM' or $this->userGroup == 'KSA') {
					$statusPusat = '<a class="btn btn-sm btn-info" title="Approve" onclick="prosesData('."'".$row["id"]."'".', '."'Plafon'".','."'Approve'".')" id="approve"><i class="fa fa-eye"></i> Approve</a>&nbsp;&nbsp;<a class="btn btn-sm btn-warning" title="Reject" onclick="prosesData('."'".$row["id"]."'".', '."'Plafon'".','."'Reject'".')" id="reject"><i class="fa fa-eye"></i> Reject</a>&nbsp;&nbsp;<button type="button" class="btn btn-sm btn-default" href="javascript:void(0)" title="Lihat"  onclick="view('."'".$row["KodePelanggan"]."'".','."'".$row["TotalValue"]."'".')"><i class="fa fa-eye"></i> Detail</button>';
				}
				else {
					$statusPusat = "-";
				}
				// $detail = '<button type="button" class="btn btn-sm btn-default" href="javascript:void(0)" title="Lihat" onclick="view('."'".$row["id"]."'".')"><i class="fa fa-eye"></i> Detail</button>';
				// $approval = '<button type="button" class="btn btn-sm btn-default" href="javascript:void(0)" title="View Approval" onclick="approval('."'".$row["id"]."'".')"><i class="fa fa-eye"></i> Lihat</button>';
				
				$output['data'][]=array($statusPusat,$row["NoOrder"],
					$row["OrderDate"],$row["KodePelanggan"],$row["NamaPelanggan"],$row["Salesman"],
					$row["Acu"],$row["CaraBayar"],"<p align='right'>".number_format($row["TotalValue"])."</p>",
					"<p align='right'>".number_format($row["plafon"])."</p>","<p align='right'>".number_format($row["sisaplafon"])."</p>");
				$nomor_urut++;
			}
		}
		else{
	...
	// view detail pelanggan - @DnA
	public function listDataDetailPelanggan()
	{
		$kode = $_POST['no'];
		$data_api = json_decode($this->Model_approval->getDataDetailPelanggan($kode),TRUE);
		$query = $data_api['output'];
    ...
    // Proses Data Approval Order Khusus - @DnA
    public function prosesDataApprovalOrderKhusus($status = NULL)
	{          
        if ($this->logged) 
		{	
			$NoOrder = $_POST['No'];
			$tipe = $_POST['tipe'];
			$valid = $this->Model_approval->prosesDataApprovalOrderKhusus($NoOrder,$tipe);
			return $valid;
		}
		else 
		{	
